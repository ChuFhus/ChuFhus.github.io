...
6. 互连线延迟的计算
  ...
  - Elmore定理/Elmore延时
    ...
    - 对于RC树形网络:
      ...
      - 计算示例: ![Elmore延时](./images/Elmore延时.png "Elmore延时示例计算")
  - 分布模型延时计算
    - 若电路参数沿导线长度L均匀分布:
      - r为单位长度电阻,c为单位长度电容
    - 一阶近似Elmore公式:
      - $\tau_N=\Delta L^2crN(N+1)/2$
      - 当N接近无穷大时,N+1和N很接近, $\tau_N=rcL^2/2=\frac{1}{2}RC$
  - 如何减少连线延迟:
    - 在连线中插入缓冲驱动器/适当对连线分段
  - 传输线模型: 最精确/但过于复杂
7. CMOS反相器的开关功耗
  - CMOS反相器的功耗包含:
    - 静态功耗:
      - 构成:
        - 亚阈值漏电(最主要):可以通过降低阈值电压来减小
        - 栅极隧穿电流
        - 反偏PN结电流
      - 但随着工艺进步,CMOS反相器的静态功耗逐渐变大 -> 促使FinFET诞生
    - 动态/开关功耗:
      - 由于输入信号变化过程中,电路不断对负载电容充放电造成
    - 短路功耗:
      - 由于输入信号短暂处于中间电平时,NMOS和PMOS同时导通,直接从VDD到GND流过电流造成
      - 产生原因:
        - 输入不可能是阶跃信号
        - VTC曲线存在有限增益的过渡区
      - 若输出信号为阶跃信号,这一项为0
  ...
  - CMOS反相器开关功耗:
    - 计算结论:
      - $P_{avg}=\frac{1}{T}C_{load}V_{DD}^2=C_{load}V_{DD}^2f$
    - 分析:
      - 
